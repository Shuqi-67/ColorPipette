...
def init_spixel_grid(args,  b_train=True):
    ...
    torch_spix_idx_tensor = torch.from_numpy(
                np.tile(spix_idx_tensor, (args.batch_size, 1, 1, 1))).type(torch.float)
    ...
    all_XY_feat = (torch.from_numpy(
        np.tile(coord_tensor, (args.batch_size, 1, 1, 1)).astype(np.float32)))
...
def poolfeat(input, prob, sp_h=2, sp_w=2):
    ...
    feat_ = torch.cat([input, torch.ones([b, 1, h, w])], dim=1)  # b* (n+1) *h*w
...
def assign2uint8(assign):
    ...
    red = torch.cat([torch.ones(size=assign.shape),  torch.zeros(size=[b,2,h,w])],dim=1)

    green = torch.cat([ torch.zeros(size=[b,1,h,w]),
                      torch.ones(size=assign.shape),
                      torch.zeros(size=[b,1,h,w])],dim=1)

    blue  = torch.cat([torch.zeros(size=[b,2,h,w]),
                       torch.ones(size=assign.shape)],dim=1)

    black = torch.zeros(size=[b,3,h,w])
    white = torch.ones(size=[b,3,h,w])
    # up probablity
...
def rgb2Lab_torch(img_in, mean_values = None):
    ...
    img= (img_in.clone() + mean_values).clamp(0, 1)
    ...
    xyz_ref_white = torch.tensor([0.95047, 1., 1.08883])
...
def label2one_hot_torch(labels, C=14):
    ...
    one_hot = torch.zeros(b, C, h, w, dtype=torch.long)
